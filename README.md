...
明明已经把一趟旅行的路线、餐厅、住宿、注意事项都整理好了，发出去却是一篇繁琐的文档。信息是全的，心情却不见了。

又或者，你脑子里已经有了“从哪到哪、几天几夜、预算大概多少”，但一想到还要做路书规划，就先心累了。

如果你也有过这种瞬间，这个 skill 就是为你准备的。

它会把“旅行计划 / 第一人称游记”，直接做成一个真正值得分享的旅行路书网页。
...
- 极致靠谱：不是只会“搜几篇攻略拼一拼”，而是会主动多角度搜索、核定信息置信度、结合地图和天气去排更合理的路线
- 极致贴心：会按段落和场景去选图，也会主动给出适合页面气质的 Pixabay 音乐方向，避免网页空有壳子没有氛围

## 2.0 更新了什么？

如果说 1.0 更像一个很会做漂亮网页的旅行路书助手，
那 2.0 开始，它更像一个真正能从想法走到成品的“旅行视觉策划搭子”。

这次升级，主要把三件事补齐了：

- 更会规划了：不一定要先写好完整路书。你只要说清楚“从哪到哪、玩几天、预算多少、偏轻松还是偏特种兵”，它就会先按用户画像挑规划模板，再去做多角度检索、地图点位校验、逐段耗时判断、天气联动和 Plan B 排布，最后再落成网页。
- 更会审美把关了：现在不是“地点对了就算过关”。2.0 把首屏与次屏过渡、页面呼吸感、时间轴节点、黑胶播放器、图片 section-level 匹配、音乐氛围建议这些我们反复打磨过的规则，都正式写进了 skill。它不只会做页面，还会主动避开那种很重的模块感和 AI UI 味。
- 更适合长期复用了：这次我们还把整个 skill 结构重构成了渐进式披露。主文件更轻，只负责入口和路由；planning、writing、visual、media、QA 都按需分流到 references。以后不管是你自己继续做新目的地，还是交给别的 agent 调用，都会更稳、更清楚。

另外，这一版还补了一条很关键、但过去很容易被忽略的首屏规则：

- 首屏更稳了：Hero 不只要“有感觉”，还要“看得清”。现在 skill 会强约束首页背景图和标题字色必须有足够区分度，必要时主动用阴影或遮罩修正；如果标题文案天然不对称，也必须做光学居中，实在救不回来时，会直接建议改文案，而不是硬做一个看起来歪掉的首页。

一句话概括 2.0：
它不再只是“帮你把旅行文档做成网页”的 skill，
而是一套从旅行意图理解、行程规划、素材选择，到最终网页落地的完整工作流。
...
它会先帮你把路书结构、搜索记录、信息置信度、地图动线、天气替代、图片氛围、标签语义和音乐交互理顺，再把它们落成一个真正可用的旅行站点。

更具体一点，它会把这件事拆成一条更稳的流水线：

- 先多角度搜：不是只搜一个“XX 攻略”，而是同时看避坑、出片、本地人推荐、雨天 Plan B
- 再判信息靠不靠谱：把官方源、地图状态、稳定网页和 UGC 分开看，避免把一条爆款笔记当事实
- 再结合地图和天气排路线：尽量少折返、少重交通，把更吃天气和光线的点放到合适时段
- 再按 section 去配图和配乐：不是随机抓城市风景，而是按 Hero / Day / beat 逐段匹配，并主动推荐 Pixabay 音乐方向
